...
describe("DocumentPipeline", function () {
  ...
  describe("Initialization", function () {
    ...
    it("should throw error if processing before initialization", async function () {
      ...
      const result = await uninitializedPipeline.processDocument(
        testFile,
        "test-topic"
      );
  ...
  describe("Embedding", function () {
    it("should generate embeddings for chunks", async function () {
      ...
      const result = await pipeline.processDocument(
        testFile,
        "test-topic-embeddings"
      );

      expect(result.success).to.be.true;
      expect(result.stages.embedding).to.be.true;
      expect(result.metadata.chunksEmbedded).to.equal(
        result.metadata.chunksCreated
      );
    });
  ...
  describe("Vector Storage", function () {
    it("should store chunks in vector store", async function () {
      ...
      expect(result.success).to.be.true;
      expect(result.stages.storing).to.be.true;
      expect(result.metadata.chunksStored).to.equal(
        result.metadata.chunksCreated
      );
    });
  ...
  describe("Custom Options", function () {
    it("should respect custom chunking options", async function () {
      ...
      const smallChunks = await pipeline.processDocument(
        testFile,
        "test-topic-small",
        {
          chunkingOptions: {
      ...
      const largeChunks = await pipeline.processDocument(
        testFile,
        "test-topic-large",
        {
          chunkingOptions: {
